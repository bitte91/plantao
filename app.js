document.addEventListener('DOMContentLoaded', () => {
            ...
            const renderFinances = (element) => {
                const totalPaid = db.shifts.filter(s => s.isPaid).reduce((sum, s) => sum + s.value, 0);
                const totalPending = db.shifts.filter(s => !s.isPaid).reduce((sum, s) => sum + s.value, 0);

                element.innerHTML = `
                    <div class="tab-content space-y-6">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            <h2 class="text-2xl font-bold text-text-primary dark:text-text-primary-dark">Dashboard Financeiro</h2>
                             <button onclick="window.openShiftModal(new Date().toISOString().split('T')[0])" class="bg-primary hover:bg-primary-600 dark:bg-primary-dark text-white px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2">
                                <i data-lucide="plus"></i> Adicionar Plantão
                            </button>
                        </div>
                        ...
                        <div class="grid md:grid-cols-2 gap-6">
                            ...
                            <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 card-hover">
                                <div class="flex items-center justify-between mb-4">
                                    ...
                                    <span class="text-sm text-yellow-600 dark:text-yellow-400 font-medium">${db.shifts.filter(s => !s.isPaid).length} pendentes</span>
                                </div>
                                <div>
                                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Valores a Receber</h3>
                                    <p class="text-3xl font-bold text-gray-900 dark:text-white">${totalPending.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                            <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Todos os Plantões</h3>
                            </div>
                            <div id="finances-table-container">
                                ${renderFinancesTable()}
                            </div>
                        </div>
                    </div>
                ...
                if(sortedShifts.length === 0) {
                    return `
                        <div class="p-8 text-center">
                            ...
                            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">Nenhum plantão registrado</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400">Adicione seu primeiro plantão para começar a acompanhar suas finanças.</p>
                        </div>
                    `;
                }
                                    ...
                                    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                                        <div class="flex-1 mb-4 md:mb-0">
                                            <div class="flex items-center gap-3">
                                                <p class="font-semibold text-gray-900 dark:text-white">${patient?.name || 'Paciente não encontrado'}</p>
                                                <span class="text-xs text-gray-500 dark:text-gray-400">${shiftDate.toLocaleDateString('pt-BR', {day: '2-digit', month: 'short', year: 'numeric'})}</span>
                                            </div>
                                        ...
                                        <div class="flex items-center gap-4">
                                            <button onclick="window.openShiftModal('${shift.date}', '${shift.id}')" class="text-gray-500 hover:text-primary dark:text-gray-400 dark:hover:text-primary-dark p-2 rounded-full transition-colors">
                                                <i data-lucide="edit-3" class="w-4 h-4"></i>
                                            </button>
                                            <label for="payment-toggle-${shift.id}" class="flex items-center cursor-pointer">
                                                <div class="relative">
                                                    <input type="checkbox" id="payment-toggle-${shift.id}" class="sr-only" ${shift.isPaid ? 'checked' : ''} onchange="window.togglePayment('${shift.id}')">
                                                    <div class="block w-14 h-8 rounded-full ${shift.isPaid ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}"></div>
                                                    <div class="dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform ${shift.isPaid ? 'translate-x-6' : ''}"></div>
                                                </div>
